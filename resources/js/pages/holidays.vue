...
export default {
    ...
    methods:{
    	...
    	async storeOrUpdate () {
            // Submit the form.
            if(this.updateData) {
                const response = await this.form.put('/api/holidays/'+this.form.id);
                if (response.status === 200) {
                    this.cleanForm();
                    showMessage(response.status, 'Project updated successfully');
                }
            }else {
                const response = await this.form.post('/api/holidays');
                if (response.status === 200) {
                    this.cleanForm();
    ...
    created(){
        var self = this;
        axios.get('/api/holidays/create').then(function (response) {
